...
void tweak_constants(spx_ctx *ctx)
{
    ...
    memcpy((unsigned char *)ctx->tweaked512_rc64, (unsigned char *)haraka512_rc64, 40*16);
...
void haraka_S_inc_init(unsigned char *s_inc)
{
    size_t i;
...
void haraka_S_inc_absorb(unsigned char *s_inc, const unsigned char *m, size_t mlen,
        const spx_ctx *ctx)
{
    size_t i;
    ...
    while (mlen + s_inc[64] >= HARAKAS_RATE) {
        ...
        m += HARAKAS_RATE - (unsigned char)s_inc[64];
        s_inc[64] = 0;
    ...
    s_inc[64] += (unsigned char)mlen;
}

void haraka_S_inc_finalize(unsigned char *s_inc)
{
...
void haraka_S_inc_squeeze(unsigned char *out, size_t outlen, unsigned char *s_inc,
        const spx_ctx *ctx)
{
    size_t i;
    ...
    for (i = 0; i < outlen && i < s_inc[64]; i++) {
        ...
        out[i] = (unsigned char)s_inc[(HARAKAS_RATE - s_inc[64] + i)];
    }
    out += i;
    outlen -= i;
    s_inc[64] -= (unsigned char)i;
    ...
    while (outlen > 0) {
        ...
        out += i;
        outlen -= i;
        s_inc[64] = (unsigned char)(HARAKAS_RATE - i);
    }
}
